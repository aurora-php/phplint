...
/**
	...
	the keys implement the {@link Hashable} interface, so that keys provide the
	...
	provided by the {@link Hash} class are applied to calculate the hash, while
	the function {@link Equality::areEqual()} provides a quite weak, surrogate
	concept of equality.
	...
	@version $Date: 2012/04/02 09:11:28 $
*/
class HashMap implements \Countable, \Iterator, Printable, Comparable {
	...
	/*
		Holds one slot for each dinstinct hash value. First index is the hash
	...
	*/
	...
	/**
		...
		@return bool True if at least one (key,element) pair has the an element
	...
	*/
	...
	/*. string .*/ function __toString()
	{
		...
		for($i = 0; $i < count($pairs); $i++){
			if( $i > 0 )
				$s .= ", ";
			$s .= "(" . \it\icosaedro\utils\TestUnit::dump($e[$i][0])
				. \it\icosaedro\utils\TestUnit::dump($e[$i][1]) . ")";
		}
		return __CLASS__ . "($s)";
	}

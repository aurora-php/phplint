...
/**
	...
	@version $Date: 2012/02/09 10:12:47 $
*/
class Types {
	...
	/**
		...
		TypeInterface} interface, and then provides the check() method that
	...
	*/
	static function parseType($type_descr)
	{
		...
		if( strpos($type_descr, "[") === FALSE ){
			...
			if( $a[0] === "array" ){
				...
				$type = self::parseType($a[count($a)-1]);
				$i1 = 1;
				$i2 = count($a) - 2;
			} else {
				// New syntax "T[]".
	...
	static function typeOf($v)
	{
		if( is_null($v) )
			...
			if( $found_index_int and ! $found_index_string )
				# FIXME: fix dummy object
				return self::parseType("array[int]object");
			else if( $found_index_string and ! $found_index_int )
				# FIXME: fix dummy object
				return self::parseType("array[string]object");
			else
				# FIXME: fix dummy object
				return self::parseType("array[]object");
		} else {
